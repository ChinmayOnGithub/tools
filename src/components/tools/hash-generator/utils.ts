/*
 * Safe pure-JS implementation of MD5.
 */
...
/*
 * Dynamic asynchronous hash generation wrapper using Web Crypto for SHA algorithms
 * and pure JS for MD5.
 */
export async function generateHash(text: string, algorithm: string): Promise<string> {
  const normalizedAlg = algorithm.toUpperCase().trim();
  
  if (normalizedAlg === 'MD5') {
    return md5(text);
  }

  let standardAlg = normalizedAlg;
  if (normalizedAlg === 'SHA1') standardAlg = 'SHA-1';
  if (normalizedAlg === 'SHA256') standardAlg = 'SHA-256';
  if (normalizedAlg === 'SHA384') standardAlg = 'SHA-384';
  if (normalizedAlg === 'SHA512') standardAlg = 'SHA-512';

  const validAlgs = ['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512'];
  if (!validAlgs.includes(standardAlg)) {
    throw new Error(`Unsupported hash algorithm: ${algorithm}`);
  }

  const globalCrypto = (globalThis as unknown as { crypto?: Crypto }).crypto;
  const cryptoRef = typeof window !== 'undefined' ? window.crypto : globalCrypto;
  if (!cryptoRef) {
    throw new Error('Web Crypto API is not available in this environment.');
  }

  const msgUint8 = new TextEncoder().encode(text);
  const hashBuffer = await cryptoRef.subtle.digest(standardAlg, msgUint8);
